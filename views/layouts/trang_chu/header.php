<!-- Image and text -->
<nav class="navbar bg-dark d-flex">
    <div class="col-3 d-flex ">
      <a class="btn" href="./index.php">
        <img src="../asset/img/logo.png" width="50" height="50" class="d-inline-block align-top" alt="" loading="lazy">
      </a>
      <a href="./index.php" class="btn-content-logo text-light mt-1" style="font-size:1.5rem"><h3 class="btn-content-logo-text">Barista</h3></a>
    </div>

    <div class="col-5 d-flex">
        
    </div>

    <div class="col-2"></div>

    <div class="col">
      <button type="button" class="btn-dang-nhap btn-dangnhap-dangky-navbar" data-toggle="modal" data-target="#form-dang-nhap">
        Đăng nhập
        <span></span>
        <span></span>
        <span></span>
        <span></span>
      </button>
      <span class="text-light">|</span>
      <button type="button" class="btn-dang-ky btn-dangnhap-dangky-navbar" data-toggle="modal" data-target="#form-dang-ky">
        Đăng ký
        <span></span>
        <span></span>
        <span></span>
        <span></span>
      </button>
    </div>
</nav> 
